...
class Game:
    ...
    def process_move(self, move):
        ...
        for combo in self._winning_combos: # combo will be each combination of triples
            tictactoe = 0
            for square in combo: # square being (row, col)
                if self._current_moves[square[0]][square[1]].label == move_label:
                    tictactoe += 1 # if another square from combo has same label, tictactoe +1
                
                if tictactoe == 3: # winning combo
    ...
    def is_tied(self):
        ...
        all_moves = True
        for row in self._current_moves: # check if every square has a label. if so and no winner, tie
            for square in row:
                if square.label == "": # if every square has label, this will stay true until end
                    all_moves = False
        
        return not(self._has_winner) and all_moves
